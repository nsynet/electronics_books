...
unsigned int   giADC_DataSave[11] ;        //ADC×ª»»µÄÊı¾İ
...
void display_speech_ADC_temperature(void){
	...
	if(G_ADC_flag){                       //ÅĞ¶ÏÊÇ·ñÓĞÎÂ¶È²ÉÑù 1£¬ÓĞ 0£¬ÎŞ
        ...
        if((int)fT != siTlast){     //ÅĞ¶ÏÎÂ¶ÈµÄÕûÊıÖµÊÇ·ñ±ä»¯
        	temperature_speech_index(guiLED_Value);  	  //ÎÂ¶ÈÕûÊıÖµÓïÒô²¥·Å
        ...
        stPID.Proportion = 1.5;              //  ÉèÖÃPID±ÈÀıÖµ
        stPID.Integral   = 0.5;              //  ÉèÖÃPID»ı·ÖÖµ
        stPID.Derivative = 0.0;              //  ÉèÖÃPIDÎ¢·ÖÖµ
        fOut = PIDCalc ( &stPID,(int)(fT*10) );        //  PID¼ÆËã
        active();	
   }   
} 
//============================================================================================
...
void temperature_speech_play(void)
{
  ...
  SP_INT_IRQ6();
   ...
   if(iAddrSpeech < speech_end_address(iaSpeech_index[iNumber]))//ÅĞ¶ÏÊÇ·ñÎª×ÊÔ´Êı¾İµÄ½áÊøµØÖ·
